...
function welcomeScreen(){
 ...
 return el('div',{class:'welcome-screen'},el('div',{class:'welcome-card'},el('span',{class:'brand-mark'},'›_'),el('h1',{},'ProfessorGito'),el('p',{class:'muted'},'A fictional Cinder investigation. Choose a mission, then a terminal username, to begin.'),el('h2',{class:'welcome-subhead'},'Mission'),missionGrid,form));
}
function finalPanel(){
...
function titleScreen(){
 ...
 return el('div',{class:'title-screen'},el('main',{class:'title-card'},el('div',{class:'title-mark','aria-hidden':'true'},'›_'),el('div',{class:'eyebrow'},'CINDER INVESTIGATION WORKSPACE'),el('h1',{class:'title-logo'},'Professor',el('span',{class:'title-gito'},'Gito')),el('p',{class:'title-tag'},scen?.tagline||'A terminal investigation thriller.'),el('button',{type:'button',class:'primary title-btn',onClick:start},started?'Continue':'New investigation'),started?button('Start a new campaign',()=>{if(confirm('Start over? Export your save first to retain this campaign. Your username and preferences will be kept.')){boot.view='game';newCampaign();}},'title-alt'):el('p',{class:'title-note muted'},'Pick a username, then investigate by typing commands. Progress saves automatically on this browser.'),el('p',{class:'title-sim'},'● LOCAL SIMULATION · NO NETWORK · NO ACCOUNTS')));
}
function render(){
 ...
 const header=el('header',{class:'topbar'},el('div',{class:'brand-group'},sidebarToggle,el('a',{class:'brand',href:'#',onClick:e=>e.preventDefault()},el('span',{class:'brand-mark'},'›_'),el('span',{},'ProfessorGito',el('small',{},'CINDER / INVESTIGATION WORKSPACE')))),el('div',{class:'top-actions'},el('span',{class:'simulation-badge'},'● LOCAL SIMULATION'),saveMenu,button('Settings',settings),file));
